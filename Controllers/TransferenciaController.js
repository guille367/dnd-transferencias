app.controller('transferenciaController', ['$scope', '$controller', 'bienService', function ($scope, $controller,bienService) {

    angular.extend(this, $controller('bienController', { $scope: $scope }));

    $scope.SinOrigen = false;
    $scope.IdHidden = 'RemitoVigente.BienesTransferenciaDC[{index}].BienDC.Id'
    ...
    $scope.InstitucionalDeOrigenCargado = function () {
        return $('#InstitucionalPatrimonioOrigenDC_Id').val() != '';
    }

    $scope.SetOrigenToFilter = function () {
        $scope.SinOrigen = false;
        $scope.bienesFilter.IdInstitucionalPatrimonio = parseInt($('#InstitucionalPatrimonioOrigenDC_Id').val());
    }
}]);